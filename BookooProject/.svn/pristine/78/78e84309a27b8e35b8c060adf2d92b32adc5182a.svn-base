...
@Component("wwm")
public class WriterWeekManager {
	...
	public static void main(String[] args) {
		...
		WriterWeekManager ww = (WriterWeekManager)app.getBean("wwm");
		ww.WriterWeekTop100();
	...
	public List<WriterWeekVO> WriterWeekTop100(){
		List<WriterWeekVO> wwlist = new ArrayList<WriterWeekVO>();
			...
			int k=1;
				...
				for(int j=0; j<wCategory.length; j++){
					...
					for(k=1; k<=15; k++){
						...
						Document doc = Jsoup.connect("http://bookdb.co.kr/bdb/PersonDictionary.do?_method=authorList&sc.page="+k+"&sc.sortType=2&sc.writerCategoryNo="+wCategory[j]).get();
					/*
	 ...
	 * 
					...
					try {
						...
						//vo값 채움		
								...
								WriterWeekVO wvo = new WriterWeekVO();
								
								wvo.setWwcate(wCategory[j]);
								//랭크는 누적 판매수로 정렬하면 나오긴 한데..편하게 쓰기 위해..
								wvo.setWwrank(wrank);
								wvo.setWwno(Integer.parseInt(wno));
								wvo.setWwname(wname);
								wvo.setWwpic(wpic);
								wvo.setWwpubbs(Integer.parseInt(wpubooks));
								wvo.setWwweek(Integer.parseInt(waccbs));
								wvo.setWwintro(wintro);

							
									wvo.setWwbisbn1(wbisbn1);
									wvo.setWwbtitle1(wb_titles1);
									wvo.setWwbimg1(wbimg1);										
								
									wvo.setWwbisbn2(wbisbn2);
									wvo.setWwbtitle2(wb_titles2);
									wvo.setWwbimg2(wbimg2);
								
									wvo.setWwbisbn3(wbisbn3);
									wvo.setWwbtitle3(wb_titles3);
									wvo.setWwbimg3(wbimg3);
								
									wvo.setWwbisbn4(wbisbn4);
									wvo.setWwbtitle4(wb_titles4);
									wvo.setWwbimg4(wbimg4);
								
									wvo.setWwbisbn5(wbisbn5);
									wvo.setWwbtitle5(wb_titles5);
									wvo.setWwbimg5(wbimg5);								
					
								wwlist.add(wvo);
								dao.writerWeekInsert(wvo);
							//}
						} // 1 : 1~~10 11~20 21~30
			...
			System.out.println("WriterWeek100 : " + ex.getMessage());
			ex.printStackTrace();
