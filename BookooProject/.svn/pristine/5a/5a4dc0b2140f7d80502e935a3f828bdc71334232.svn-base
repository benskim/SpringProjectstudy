...
import com.sist.janre.dao.JanreListDAO;
import com.sist.janre.dao.RentVO;
...
public class MainController {
	...
	public String main_genre(String cage, String ctheme, Model model){
		List<BookListVO> list = new ArrayList<BookListVO>();
		List<RentVO> rlist = new ArrayList<RentVO>();
		String testa = "7";
		String testt = "역사";
		
		if(cage==null) cage="20";
		if(ctheme==null)
		{
			ctheme="종교";
			//list = janredao.BasicJanreList();
			rlist = janredao.A_T_JanreList(testa, testt);
			list  = bldao.assistJanre(ctheme);
		}
		if(cage!=null&&ctheme!=null)
		{
			rlist = janredao.A_T_JanreList(cage, ctheme);
			list  = bldao.assistJanre(ctheme);
		}
		else if(cage!=null&&ctheme==null)
		{
			rlist = janredao.A_JanreList(cage);
			list  = bldao.assistJanre(ctheme);
		}
		else if(cage==null&&ctheme!=null)
		{
			rlist = janredao.T_JanreList(ctheme);
			list  = bldao.assistJanre(ctheme);
		}
		//정제작업 완료
		...
		{
			imglist.add(i, list.get(i).getBimg());
		}
