...
import com.sist.manager.ReviewVO;
...
public class BookDAO {
	...
	public BookDAO(){
		try {
			mc = new MongoClient(new ServerAddress(new InetSocketAddress("211.238.142.36", 27017)));
//			port 변경가능 : sudo nano mongodb.conf
//			mc = new MongoClient(localhost, 1521 - oracle);
			db=mc.getDB("mydb"); //database
			dbc=db.getCollection("Review"); //create table movie
...
#{startDt},#{endDt},#{gender},#{age},#{category})
	...
	public void bestInsert(String s){
		try {
